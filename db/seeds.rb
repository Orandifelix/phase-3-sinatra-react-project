...
# Create 10 users, each with their own profile
10.times do
  gender = rand(0..1) == 0 ? 'female' : 'male'
  avatars = gender == 'female' ? females_avatars : males_avatars
  user = create_user(gender, gender == 'female' ? 'male' : 'female')
  generate_profile(user, avatars)
end
...
def generate_favorites(users, profiles)
  ...
  users.each do |user|
    # Ensure each user has 1 favorite
    num_favorites = 1

    # Shuffle the profiles array to make the selection random
    shuffled_profiles = profiles.shuffle

    # Select the first profile from the shuffled array
    favorite_profiles = shuffled_profiles.first(num_favorites)
...
# Generate profiles and favorites
all_users.each do |user|
  avatars = user.gender == 'female' ? females_avatars : males_avatars
  generate_profile(user, avatars)
end
